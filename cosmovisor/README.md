...
```sh
go install github.com/weijun-sh/cosmos-sdk/cosmovisor/cmd/cosmovisor@latest
```
...
Next, we can add a migration - which is defined using `x/upgrade` [upgrade plan](https://github.com/weijun-sh/cosmos-sdk/blob/master/docs/core/upgrade.md) (you may refer to a past version if you are using an older Cosmos SDK release). In a migration we can do any deterministic state change. 
